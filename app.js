...
function initSupabaseClient() {
  const SUPABASE_URL = 'https://imwotvluaibgtbixbvih.supabase.co';
  const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imltd290dmx1YWliZ3RiaXhidmloIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcxMTc2NDksImV4cCI6MjA5MjY5MzY0OX0.43dMnnfbSo9Tlp7mhShPw90PEtPHwSIrsWEqYY-9KVw';
...
async function dbInsertNotification(text, icon = '🔔') {
  const userId = App.currentUser?.id;
  if (!userId) return null;
  const { data: row, error } = await _supabase.from('notifications').insert({
    user_id: userId,
    text,
    icon,
    read: false
  }).select().single();
  if (error) { console.error('[PayBack] notification insert error:', error); return null; }
...
async function addNotification(text, icon = '🔔') {
  ...
  const notif = await dbInsertNotification(text, icon);
  if (notif) {
...
async function markAsPaid(id) {
  ...
  try {
    ...
    const updated = await dbUpdateInvoice(id, { ...inv, status: 'paid', paidAt });
    const idx = App.data.invoices.findIndex(i => i.id === id);
    if (idx !== -1) App.data.invoices[idx] = updated;
    await addNotification(`${inv.number} — to'landi! ${formatCurrency(inv.amount)}`, '✅');
    showToast(`${inv.number} — to'langan deb belgilandi ✓`, 'success');
    renderPage(App.currentPage);
    updateBadge();
  } catch (err) {
    console.error('[PayBack] markAsPaid error:', err);
    showToast("Statusni yangilashda xato yuz berdi", 'error');
  }
}
...
async function init() {
  ...
  _supabase.auth.onAuthStateChange(async (event, session) => {
    if (event === 'SIGNED_OUT' && App.isLoggedIn) {
      App.isLoggedIn  = false;
      App.currentUser = null;
      App.data.clients       = [];
      App.data.invoices      = [];
